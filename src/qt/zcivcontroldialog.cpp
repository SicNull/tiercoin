...
std::list<std::string> ZTierControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTierControlDialog::listMints;

ZTierControlDialog::ZTierControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTierControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTierControlDialog::~ZTierControlDialog()
{
    delete ui;
}

void ZTierControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTierControlDialog::updateList()
{
...
void ZTierControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTierControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTier_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTierControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTierControlDialog::GetSelectedMints()
{
...
void ZTierControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
