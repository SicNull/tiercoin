                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTIER ought to be enough for anybody." - Bill Gates, 2017
    ui->zTIERpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTIERSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTIER->setEnabled(false);
        ui->pushButtonMintzTIER->setToolTip(tr("zTIER is currently disabled due to maintenance."));

        ui->pushButtonSpendzTIER->setEnabled(false);
        ui->pushButtonSpendzTIER->setToolTip(tr("zTIER is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTIERpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTIER_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTIER is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTIER...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTIER in ") +
...
void PrivacyDialog::on_pushButtonSpendzTIER_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTIER is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTIER
        sendzTIER();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTIER
    sendzTIER();
}

void PrivacyDialog::on_pushButtonZTierControl_clicked()
{
    ZTierControlDialog* zTierControl = new ZTierControlDialog(this);
    zTierControl->setModel(walletModel);
    zTierControl->exec();
}

void PrivacyDialog::setZTierControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTierSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTIER()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Tiercoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTIERpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTIER is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TIER </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTIER</b>";
    ...
    // use mints from zTier selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTierControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTierControlDialog::GetSelectedMints();
    }
    ...
    ZTierControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTier Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Tier, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTier Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTIER </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTIER "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTIER "));
    ui->labelzTIERAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTIERSyncStatus->setVisible(fShow);
}
